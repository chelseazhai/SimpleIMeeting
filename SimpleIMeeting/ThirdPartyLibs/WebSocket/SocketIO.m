...
- (void) connectToHost:(NSString *)host onPort:(NSInteger)port
{
    ...
    {
        NSLog(@"connectToHost: %@ and port = %d", host, port);
        _isConnecting = YES;
...
- (void) requestFailed:(ASIHTTPRequest *)request
{
    ...
    // add by ares
    if ([_delegate respondsToSelector:@selector(socketIODidConnectError:error:)])
    {
        [_delegate socketIODidConnectError:self error:[error localizedDescription]];
    }
}
